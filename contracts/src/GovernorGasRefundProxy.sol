...
contract GovernorGasRefundProxy {
  ...
  function castVoteBySig(address _target, uint256 _proposalId, uint8 _support, uint8 _v, bytes32 _r, bytes32 _s) public returns (uint256) {
    ...
    require(balance >= gasCost, "no budget available to refund gas");
    ...
    require(success, "failed to refund gas cost");

    return votes;
  }

  /* function castVoteBySigWithRefferal(address _target, uint256 _proposalId, uint8 _support, uint8 _v, bytes32 _r, bytes32 _s) { */
  /* } */

  // Add gas setters that allow you to prove new gas costs to the contract
}
